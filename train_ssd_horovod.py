...
from dataset import dataset_common_hvd
...
tf.app.flags.DEFINE_integer(
    'num_cpu_threads', (p.num_cores_per_socket()*p.num_cpu_sockets()),   # set intra_op_parallelism_threads to number of physical cores
...
tf.app.flags.DEFINE_integer(
    'max_number_of_steps', 7500,  # Divide single worker max_number_of_steps by 16
    'The max number of steps to use for training.') 
tf.app.flags.DEFINE_integer(
    'batch_size', 32,             # Retain single worker batch size for weak scaling
...
tf.app.flags.DEFINE_float('learning_rate', 0.016, 'Initial learning rate.')
tf.app.flags.DEFINE_float(
    'end_learning_rate', 0.000001,
    'The minimal end learning rate used by a polynomial decay learning rate.')
# for learning rate piecewise_constant decay
tf.app.flags.DEFINE_string(
    'decay_boundaries', '5000, 6250',   # Divide single worker decay_boundaries by 16
...
tf.app.flags.DEFINE_float(
                   'warmup_end_learning_rate', 0.016,
                   'End learning rate for warmup stage.')
tf.app.flags.DEFINE_integer(
                    'warmup_steps', 5000,
                    'Number of steps for warmup stage.')
...
def input_pipeline(dataset_pattern='train-*', is_training=True, batch_size=FLAGS.batch_size):
    def input_fn():
        ...
        local_seed = 1234 + int(hvd.rank()) # ensure each worker gets a different batch
        image, _, shape, loc_targets, cls_targets, match_scores = dataset_common_hvd.slim_get_batch(FLAGS.num_classes,
                                                                                batch_size,
...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.device('/cpu:0'):
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                # Horovod all reduce
                n_positives = hvd.allreduce(n_positives, average=True)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        if FLAGS.warmup_steps > 0:
            ...
            print("Warmup steps:", FLAGS.warmup_steps,flush=True)
            print("Warmup start LR:", FLAGS.warmup_start_learning_rate,flush=True)
            print("Warmup end LR:", FLAGS.warmup_end_learning_rate,flush=True)

        def warmup_decay(global_step, warmup_start_lr, warmup_end_lr, warmup_steps):

               p = tf.cast(global_step, tf.float32)/tf.cast(warmup_steps, tf.float32) 
               diff = math_ops.subtract(warmup_end_lr, warmup_start_lr)
               res =  math_ops.add(warmup_start_lr, math_ops.multiply(diff,p))
               return res

        truncated_learning_rate_warmup = tf.cond(global_step < FLAGS.warmup_steps,
                                            lambda: warmup_decay(global_step, FLAGS.warmup_start_learning_rate, FLAGS.warmup_end_learning_rate, FLAGS.warmup_steps),
                                            lambda: truncated_learning_rate )
       
        # Create a tensor named learning_rate for logging purposes.                                                                                                                                    
        truncated_learning_rate_warmup=tf.identity(truncated_learning_rate_warmup,name="learning_rate_log")
        

        optimizer = tf.train.MomentumOptimizer(learning_rate=truncated_learning_rate_warmup,
...
def main(_):
    ...
    ssd_detector = tf.estimator.Estimator(
        model_fn=replicate_ssd_model_fn, model_dir=model_dir_hvd, config=run_config,
        params={
            'num_gpus': num_gpus,
    ...
    ssd_detector.train(input_fn=input_pipeline(dataset_pattern='train-*', is_training=True, batch_size=FLAGS.batch_size),
                    hooks=[logging_hook, bcast_hook], max_steps=FLAGS.max_number_of_steps)
